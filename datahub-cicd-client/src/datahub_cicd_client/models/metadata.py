...
class GlobalTag(BaseModel):
    ...
    class Config:
        ...
        json_schema_extra = {
            "example": {"urn": "urn:li:tag:PII", "name": "PII", "color": "#FF5733"}
        }
...
class EntityMetadata(BaseModel):
    ...
    class Config:
        ...
        json_schema_extra = {
            "example": {
                "owners": [
                    {"urn": "urn:li:corpuser:john.doe", "type": "corpuser", "name": "John Doe"}
                ],
                ...
                "global_tags": [{"urn": "urn:li:tag:PII", "name": "PII", "color": "#FF5733"}],
...
class Relationship(BaseModel):
    ...
    direction: str = Field(
        default="OUTGOING",
        regex=r"^(OUTGOING|INCOMING|UNDIRECTED)$",
...
class SchemaField(BaseModel):
    ...
    glossary_terms: List[GlossaryTerm] = Field(
        default_factory=list, description="Field-level glossary terms"
    )
...
class UsageStatistics(BaseModel):
    ...
    user_counts: List[Dict[str, Any]] = Field(
        default_factory=list, description="User activity counts"
    )
    ...
    class Config:
        ...
        json_schema_extra = {
            "example": {
                ...
                "top_sql_queries": ["SELECT * FROM customers WHERE status = 'active'"],
            }
        }
