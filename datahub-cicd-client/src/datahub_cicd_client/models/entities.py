...
class DataContract(BaseDataHubEntity):
    ...
    status: Optional[str] = Field(
        None, regex=r"^(ACTIVE|INACTIVE|DRAFT)$", description="Contract status"
    )
...
class Assertion(BaseDataHubEntity):
    ...
    assertion_type: str = Field(
        ..., regex=r"^(FIELD|DATASET|CUSTOM)$", description="Assertion type"
    )
...
class Test(BaseDataHubEntity):
    ...
    status: Optional[str] = Field(
        None, regex=r"^(PASSED|FAILED|SKIPPED)$", description="Test status"
    )
...
class StructuredPropertyDefinition(BaseDataHubEntity):
    ...
    value_type: str = Field(
        ..., regex=r"^(STRING|NUMBER|BOOLEAN|DATE|URN)$", description="Value type"
    )
    cardinality: str = Field(
        default="SINGLE", regex=r"^(SINGLE|MULTIPLE)$", description="Cardinality"
    )
    allowed_values: Optional[List[str]] = Field(None, description="Allowed values for the property")
    entity_types: List[str] = Field(
        default_factory=list, description="Entity types this property applies to"
    )
    ...
    class Config:
        ...
        json_schema_extra = {
            "example": {
                ...
                "platform": "postgres",
                "type": "postgres",
                "config": {"host": "prod-db.company.com", "database": "production"},
                "schedule": "0 2 * * *",
                "enabled": True,
...
class IngestionExecution(BaseModel):
    ...
    status: str = Field(
        ..., regex=r"^(RUNNING|SUCCESS|FAILURE|CANCELLED)$", description="Execution status"
    )
