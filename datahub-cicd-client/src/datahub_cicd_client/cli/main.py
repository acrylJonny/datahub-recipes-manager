...
class DataHubCLIClient:
    ...
    def __init__(self, server_url: str, token: str, verify_ssl: bool = True, timeout: int = 30):
        ...
        self.connection = DataHubConnection(
            server_url=server_url, token=token, verify_ssl=verify_ssl, timeout=timeout
        )
    ...
    def list_structured_properties(self, query="*", start=0, count=100):
        return self.properties_service.list_structured_properties(
    ...
    def list_glossary_terms(self, node_urn=None, query=None, count=100, start=0):
        return self.glossary_service.list_glossary_terms(
            node_urn=node_urn, query=query, count=count, start=start
        )
    ...
    def create_glossary_term(
        self, term_id, name, description="", parent_node_urn=None, term_source="INTERNAL"
    ):
        return self.glossary_service.create_glossary_term(
            term_id, name, description, parent_node_urn, term_source
        )
    ...
    def get_comprehensive_glossary_data(self, query="*", start=0, count=100):
        return self.glossary_service.get_comprehensive_glossary_data(
            query=query, start=start, count=count
        )
    ...
    def create_data_product(
        self, product_id: str, name: str, description: str = "", external_url: Optional[str] = None
    ):
        return self.data_product_service.create_data_product(
            product_id, name, description, external_url
        )
...
def get_client_from_env() -> DataHubCLIClient:
    ...
    return DataHubCLIClient(server_url=server_url, token=token, verify_ssl=verify_ssl)
...
def cmd_glossary_list_terms(client: DataHubCLIClient, args):
    """List glossary terms."""
    result = client.list_glossary_terms(
        node_urn=args.node_urn, query=args.query, start=args.start, count=args.count
    )
    output_result(result, args.format)
...
def cmd_glossary_create_term(client: DataHubCLIClient, args):
    ...
    result = client.create_glossary_term(
        args.id, args.name, args.description or "", args.parent, args.source or "INTERNAL"
    )
    if result:
...
def cmd_glossary_comprehensive(client: DataHubCLIClient, args):
    ...
    result = client.get_comprehensive_glossary_data(
        query=args.query, start=args.start, count=args.count
    )
    output_result(result, args.format)
...
def cmd_data_products_create(client: DataHubCLIClient, args):
    ...
    result = client.create_data_product(
        args.id, args.name, args.description or "", args.external_url
    )
    if result:
    ...
    if args.format == "table":
        ...
        if result:
            ...
            for source in result:
                ...
                print(
                    f"{source.get('id', '')}\t{source.get('name', '')}\t{source.get('type', '')}\t{status}\t{last_exec.get('startTimeMs', 'N/A') if last_exec else 'N/A'}"
                )
        else:
...
def cmd_ingestion_executions(client: DataHubCLIClient, args):
    ...
    if args.format == "table":
        ...
        if result:
            ...
            for exec in result:
                print(
                    f"{exec.get('id', '')}\t{exec.get('status', '')}\t{exec.get('startTimeMs', '')}\t{exec.get('durationMs', '')}\t{exec.get('requestedAt', '')}"
                )
        else:
    ...
    try:
        ...
        for item in data:
            ...
            if item_type == "tag":
                ...
                results.append(
                    {"type": "tag", "id": item["id"], "success": bool(result), "urn": result}
                )
            elif item_type == "domain":
                result = client.create_domain(
                    item["id"], item["name"], item.get("description", ""), item.get("parent")
                )
                results.append(
                    {"type": "domain", "id": item["id"], "success": bool(result), "urn": result}
                )
...
def main():
    ...
    domains_entities_parser = domains_subparsers.add_parser(
        "find-entities", help="Find entities in domain"
    )
    ...
    props_create_parser.add_argument(
        "--value-type", default="STRING", help="Value type (STRING, NUMBER, etc.)"
    )
    props_create_parser.add_argument(
        "--cardinality", default="SINGLE", help="Cardinality (SINGLE, MULTIPLE)"
    )
    ...
    # Glossary get node
    glossary_get_node_parser = glossary_subparsers.add_parser(
        "get-node", help="Get glossary node by URN"
    )
    ...
    # Glossary get term
    glossary_get_term_parser = glossary_subparsers.add_parser(
        "get-term", help="Get glossary term by URN"
    )
    ...
    glossary_create_node_parser = glossary_subparsers.add_parser(
        "create-node", help="Create glossary node"
    )
    ...
    glossary_create_term_parser = glossary_subparsers.add_parser(
        "create-term", help="Create glossary term"
    )
    ...
    glossary_delete_node_parser = glossary_subparsers.add_parser(
        "delete-node", help="Delete glossary node"
    )
    ...
    glossary_delete_term_parser = glossary_subparsers.add_parser(
        "delete-term", help="Delete glossary term"
    )
    ...
    glossary_comp_parser = glossary_subparsers.add_parser(
        "comprehensive", help="Get comprehensive glossary data"
    )
    ...
    # Data products list
    data_products_list_parser = data_products_subparsers.add_parser(
        "list", help="List data products"
    )
    ...
    data_products_list_parser.add_argument(
        "--count", type=int, default=100, help="Number of results"
    )
    data_products_list_parser.set_defaults(func=cmd_data_products_list)

    # Data products get
    data_products_get_parser = data_products_subparsers.add_parser(
        "get", help="Get data product by URN"
    )
    ...
    data_products_create_parser = data_products_subparsers.add_parser(
        "create", help="Create data product"
    )
    ...
    data_products_delete_parser = data_products_subparsers.add_parser(
        "delete", help="Delete data product"
    )
    ...
    # Ingestion create
    ingestion_create_parser = ingestion_subparsers.add_parser(
        "create", help="Create ingestion source"
    )
    ...
    ingestion_create_parser.add_argument(
        "--schedule-interval", default="0 0 * * *", help="Schedule interval (cron)"
    )
    ...
    # Ingestion delete
    ingestion_delete_parser = ingestion_subparsers.add_parser(
        "delete", help="Delete ingestion source"
    )
    ...
    # Ingestion trigger
    ingestion_trigger_parser = ingestion_subparsers.add_parser(
        "trigger", help="Trigger ingestion source"
    )
    ...
    ingestion_exec_parser = ingestion_subparsers.add_parser(
        "executions", help="Get execution history"
    )
