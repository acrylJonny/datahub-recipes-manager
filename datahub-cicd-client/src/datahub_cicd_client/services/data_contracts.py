...
from datahub_cicd_client.services.base_service import BaseInputOutputService, OperationResult, BatchOperationResult
...
class DataContractService(BaseInputOutputService):
    ...
    def get_data_contracts(self, query: str = "*", start: int = 0, count: int = 100) -> Dict[str, Any]:
        """
        ...
        """
        self.logger.info(f"Getting data contracts with query: {query}, start: {start}, count: {count}")
        ...
        try:
            ...
            if result and "searchAcrossEntities" in result:
                ...
                dataset_info = {}
                if entity_urns:
                    self.logger.info(f"Fetching dataset information for {len(entity_urns)} entities")
    ...
    def get_datasets_by_urns(self, urns: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            if result and "searchAcrossEntities" in result:
                return {
                    "success": True,
                    "data": result["searchAcrossEntities"]
                }
            else:
                error_msg = "No data returned from DataHub"
                if result and "errors" in result:
    ...
    def list_data_contracts(self, query: str = "*", start: int = 0, count: int = 100) -> List[Dict[str, Any]]:
        """
        ...
        """
        self.logger.info(f"Listing data contracts with query: {query}, start: {start}, count: {count}")
        ...
        if result.get("success", False) and result.get("data"):
            ...
            self.logger.error(f"Error listing data contracts: {result.get('error', 'Unknown error')}")
            return []
    ...
    def update_data_contract(
        ...
        operation = self.create_operation("update_data_contract", entity_data, entity_urn=contract_urn)
    ...
    def bulk_create_data_contracts(self, contracts_data: List[Dict[str, Any]]) -> BatchOperationResult:
    ...
    def import_data_contracts_from_json(self, json_data: List[Dict[str, Any]]) -> BatchOperationResult:
        """Import data contracts from JSON data."""
        return self.bulk_create_data_contracts(json_data)
    
    def export_data_contracts_to_mcps(self, contracts_data: List[Dict[str, Any]], filename: Optional[str] = None) -> Optional[str]:
