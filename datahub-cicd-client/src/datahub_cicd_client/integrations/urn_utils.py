...
def get_full_urn_from_name(entity_type: str, name: str, namespace: Optional[str] = None) -> str:
    """
...
def generate_deterministic_structured_property_id(entity_urn: str, property_name: str) -> str:
    """
...
def get_mutation_config_for_environment(environment_name: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        mutation = (
            Mutation.objects.filter(name__icontains=environment_name).first()
            or Mutation.objects.first()
        )
        ...
        logger.error(
            f"Error getting mutation config for environment '{environment_name}': {str(e)}"
        )
        return None


def apply_urn_mutations_to_entity(
    entity_data: Dict[str, Any], environment_name: str
) -> Dict[str, Any]:
    """
    ...
    for field in urn_fields:
        if field in mutated_data and mutated_data[field]:
            ...
            mutated_urn = generate_mutated_urn(
                original_urn, environment_name, entity_type, mutation_config
            )
...
def generate_tag_urn(
    input_urn: str, environment_name: str, mutation_config: Optional[Dict[str, Any]] = None
) -> str:
    """Generate a mutated tag URN."""
    return generate_mutated_urn(input_urn, environment_name, "tag", mutation_config)


def generate_glossary_term_urn(
    input_urn: str, environment_name: str, mutation_config: Optional[Dict[str, Any]] = None
) -> str:
...
def generate_structured_property_urn(
    input_urn: str, environment_name: str, mutation_config: Optional[Dict[str, Any]] = None
) -> str:
...
def apply_urn_mutations_to_associations(
    ...
    mutated_data = _mutate_glossary_term_associations(
        mutated_data, environment_name, mutation_config
    )
    mutated_data = _mutate_structured_property_associations(
        mutated_data, environment_name, mutation_config
    )
    ...
    mutated_data = _mutate_related_term_associations(
        mutated_data, environment_name, mutation_config
    )
...
def _mutate_tag_associations(
    data: Dict[str, Any], environment_name: str, mutation_config: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    if "globalTags" in data and "tags" in data["globalTags"]:
        for tag in data["globalTags"]["tags"]:
            if "tag" in tag:
                ...
                mutated_urn = generate_mutated_urn(
                    original_urn, environment_name, "tag", mutation_config
                )
                tag["tag"] = mutated_urn
        ...
        if isinstance(data["tags"], list):
            for i, tag_item in enumerate(data["tags"]):
                if isinstance(tag_item, dict):
                    ...
                    # Direct URN string
                    mutated_urn = generate_mutated_urn(
                        tag_item, environment_name, "tag", mutation_config
                    )
    ...
    if "glossaryTerms" in data and "terms" in data["glossaryTerms"]:
        for term in data["glossaryTerms"]["terms"]:
            if "urn" in term:
                ...
                mutated_urn = generate_mutated_urn(
                    original_urn, environment_name, "glossaryTerm", mutation_config
                )
    ...
    if "structuredProperties" in data and "properties" in data["structuredProperties"]:
        for prop in data["structuredProperties"]["properties"]:
            if "propertyUrn" in prop:
                ...
                mutated_urn = generate_mutated_urn(
                    original_urn, environment_name, "structuredProperty", mutation_config
                )
...
def _mutate_domain_associations(
    ...
    if "domain" in data and "domain" in data["domain"]:
        ...
        mutated_urn = generate_mutated_urn(
            original_urn, environment_name, "domain", mutation_config
        )
    ...
    if "parentNodes" in data and "nodes" in data["parentNodes"]:
        for node in data["parentNodes"]["nodes"]:
            if "urn" in node:
                ...
                mutated_urn = generate_mutated_urn(
                    original_urn, environment_name, "glossaryNode", mutation_config
                )
    ...
    if "forms" in data:
        for form in data["forms"]:
            if "formUrn" in form:
                ...
                if "structuredProperty" in original_urn and mutation_config.get(
...
def mutate_mcp_associations(
    ...
    if "aspect" in mutated_data:
        mutated_data["aspect"] = apply_urn_mutations_to_associations(
            mutated_data["aspect"], environment_name, mutation_config
        )
