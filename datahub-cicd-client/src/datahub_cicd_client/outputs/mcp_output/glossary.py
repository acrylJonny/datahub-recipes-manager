...
class GlossaryAsyncOutput(EntityRelationshipAsyncOutput, MetadataAssignmentAsyncOutput):
    ...
    def create_glossary_node_mcps(self, entity_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if GlossaryNodeKeyClass and GlossaryNodeInfoClass:
                ...
                key = GlossaryNodeKeyClass(name=entity_data.get("name", "unknown"))
                ...
                if entity_data.get("owners"):
                    ownership_mcps = self._create_ownership_mcps(
                        node_urn, entity_data["owners"], "glossaryNode"
                    )
    ...
    def create_glossary_term_mcps(self, entity_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if GlossaryTermKeyClass and GlossaryTermInfoClass:
                # Create proper DataHub MCP using schema classes
                key = GlossaryTermKeyClass(name=entity_data.get("name", "unknown"))
                ...
                if entity_data.get("owners"):
                    ownership_mcps = self._create_ownership_mcps(
                        term_urn, entity_data["owners"], "glossaryTerm"
                    )
                    mcps.extend(ownership_mcps)
    ...
    def _create_ownership_mcps(
        self, entity_urn: str, owners: List[Dict[str, Any]], entity_type: str
    ) -> List[Dict[str, Any]]:
    ...
    def delete_entity_mcps(self, entity_urn: str) -> List[Dict[str, Any]]:
        ...
        try:
            if MetadataChangeProposalClass:
                ...
                # Fallback
                mcps.append({"operation": "delete", "urn": entity_urn, "entityType": entity_type})
    ...
    def add_owner_mcps(
        ...
        return self._create_ownership_mcps(
            entity_urn, [{"owner": owner_urn, "type": ownership_type}], entity_type
        )
    ...
    def remove_from_entity_mcps(
        self, entity_urn: str, glossary_term_urn: str
    ) -> List[Dict[str, Any]]:
    ...
    def create_glossary_term_assignment_mcps(
        self, glossary_term_urn: str, entity_urns: List[str]
    ) -> List[Dict[str, Any]]:
    ...
    def create_entity_glossary_term_assignment_mcps(
        self, entity_urn: str, glossary_term_urns: List[str]
    ) -> List[Dict[str, Any]]:
        ...
        glossary_terms_mcp = self.mcp_emitter.create_mcp(
            entity_urn=entity_urn, aspect_name="glossaryTerms", aspect_value={"terms": terms}
        )
