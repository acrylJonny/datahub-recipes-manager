    ...
    def create_entity_mcps(self, entity_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if DataContractKeyClass and DataContractPropertiesClass:
                ...
                key = DataContractKeyClass(id=entity_data.get("id", "unknown"))

                properties = DataContractPropertiesClass(
                    entity=entity_data.get("entityUrn", entity_data.get("entity", "")),
                    freshness=entity_data.get("freshness"),
                    schema=entity_data.get("schema"),
                    dataQuality=entity_data.get("dataQuality"),
                )

                # Create MCP using manual conversion (to_obj() method has issues)
                mcp_dict = {
        ...
        try:
            if MetadataChangeProposalClass:
                ...
                mcp = MetadataChangeProposalClass(
                    ...
                    entityType="dataContract",
                    aspectName="status",
                    aspect=DataContractStateClass(state="REMOVED")
                    if DataContractStateClass
                    else {"state": "REMOVED"},
                ...
                # Fallback
                mcps.append(
                    {"operation": "delete", "urn": entity_urn, "entityType": "dataContract"}
                )
