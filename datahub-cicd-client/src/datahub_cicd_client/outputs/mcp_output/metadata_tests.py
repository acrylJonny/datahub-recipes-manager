...
class MetadataTestAsyncOutput(BaseAsyncOutput):
        ...
        try:
            ...
            if TestDefinitionClass and TestInfoClass:
                ...
                test_definition = TestDefinitionClass(
                    type=TestDefinitionTypeClass._from_dict({"code": entity_data.get("type", "CUSTOM")}) if TestDefinitionTypeClass else "CUSTOM"
                )
    ...
    def _create_test_result_mcps(self, test_urn: str, results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
            ...
            try:
                for result_data in results:
                    test_result = TestResultClass(
                        type=TestResultTypeClass._from_dict({"code": result_data.get("type", "SUCCESS")}) if TestResultTypeClass else "SUCCESS",
