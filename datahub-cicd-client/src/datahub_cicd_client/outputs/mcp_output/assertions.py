...
class AssertionAsyncOutput(BaseAsyncOutput):
    ...
    def create_entity_mcps(self, entity_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if AssertionKeyClass and AssertionInfoClass:
                ...
                key = AssertionKeyClass(
                    assertionId=entity_data.get("id", "unknown")
                )
                
                info = AssertionInfoClass(
                    type=AssertionTypeClass._from_dict({"code": entity_data.get("type", "DATASET")}) if AssertionTypeClass else "DATASET",
                ...
                if entity_data.get("actions"):
                    action_mcps = self._create_assertion_action_mcps(assertion_urn, entity_data["actions"])
                    mcps.extend(action_mcps)
                
                # Add assertion results if provided
                if entity_data.get("results"):
                    result_mcps = self._create_assertion_result_mcps(assertion_urn, entity_data["results"])
    ...
    def _create_assertion_action_mcps(self, assertion_urn: str, actions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
            ...
            try:
                for action_data in actions:
                    assertion_action = AssertionActionClass(
                        type=AssertionActionTypeClass._from_dict({"code": action_data.get("type", "FAIL")}) if AssertionActionTypeClass else "FAIL"
                    )
    ...
    def _create_assertion_result_mcps(self, assertion_urn: str, results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
            ...
            try:
                for result_data in results:
                    assertion_result = AssertionResultClass(
                        type=AssertionResultTypeClass._from_dict({"code": result_data.get("type", "SUCCESS")}) if AssertionResultTypeClass else "SUCCESS",
