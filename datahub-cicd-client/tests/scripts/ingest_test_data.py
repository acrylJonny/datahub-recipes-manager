...
def create_rest_emitter() -> DatahubRestEmitter:
    ...
    return DatahubRestEmitter(gms_server=gms_url, token=token if token else None)
    ...
    for dataset in datasets:
        ...
        properties_mcp = MetadataChangeProposalWrapper(
            ...
            aspect=DatasetPropertiesClass(
                ...
                customProperties={"test_property": "test_value", "platform": dataset["platform"]},
            ),
        )
        ...
        ownership_mcp = MetadataChangeProposalWrapper(
            ...
            aspect=OwnershipClass(
                owners=[
                    OwnerClass(owner="urn:li:corpuser:test_user", type=OwnershipTypeClass.DATAOWNER)
                ]
            ),
        )
...
def ingest_sample_domains(emitter: DatahubRestEmitter) -> None:
    ...
    for domain in domains:
        properties_mcp = MetadataChangeProposalWrapper(
            ...
            aspect=DomainPropertiesClass(name=domain["name"], description=domain["description"]),
        )
    ...
    for tag in tags:
        properties_mcp = MetadataChangeProposalWrapper(
            ...
            aspect=TagPropertiesClass(
                name=tag["name"], description=tag["description"], colorHex=tag["colorHex"]
            ),
        )
...
def ingest_sample_glossary(emitter: DatahubRestEmitter) -> None:
    ...
    for node in nodes:
        properties_mcp = MetadataChangeProposalWrapper(
            entityUrn=node["urn"],
            aspect=GlossaryNodeInfoClass(name=node["name"], description=node["description"]),
        )
    ...
    for term in terms:
        properties_mcp = MetadataChangeProposalWrapper(
            entityUrn=term["urn"],
            aspect=GlossaryTermInfoClass(name=term["name"], description=term["description"]),
        )
