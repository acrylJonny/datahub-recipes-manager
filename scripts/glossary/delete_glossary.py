...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Delete a glossary node or term from DataHub"
    )
    ...
    try:
        ...
        if args.entity_type == "node":
            ...
            if has_dependencies and not args.force:
                logger.error(
                    f"Glossary node '{node_name}' has child nodes or terms. Use --force to delete it and all its children."
                )
                sys.exit(1)
            ...
            if has_dependencies and not args.force:
                logger.error(
                    f"Glossary term '{term_name}' has entity associations. Use --force to delete it anyway."
                )
                sys.exit(1)
