...
from scripts.glossary.glossary_utils import load_glossary_node_from_file, load_glossary_term_from_file
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description="Create a new glossary node or term in DataHub")
    ...
    try:
        ...
        if args.entity_type == "node":
            ...
            if args.input_file:
                ...
                logger.info("Created glossary node definition from command line arguments")
            
            logger.info(f"Creating glossary node: {node_data.get('properties', {}).get('name')}")
            node_urn = client.import_glossary_node(node_data)
            
            if node_urn:
                logger.info(f"Successfully created glossary node: {node_data.get('properties', {}).get('name')}")
            ...
            if args.input_file:
                ...
                logger.info("Created glossary term definition from command line arguments")
            
            logger.info(f"Creating glossary term: {term_data.get('properties', {}).get('name')}")
            term_urn = client.import_glossary_term(term_data)
            
            if term_urn:
                logger.info(f"Successfully created glossary term: {term_data.get('properties', {}).get('name')}")
