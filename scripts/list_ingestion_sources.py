...
def list_sources(
    client: DataHubRestClient, output_format: str = "text"
) -> List[Dict[str, Any]]:
    """
        ...
        if output_format == "json":
            ...
            for source in sources:
                ...
                if (
                    "schedule" in source
                    and source["schedule"]
                    and "interval" in source["schedule"]
                ):
                    schedule_info = source["schedule"]["interval"]

                print(
                    f"{source_id:<30} | {name:<30} | {type_info:<15} | {schedule_info:<20}"
                )
...
def main():
    ...
    try:
        ...
        if not sources and not args.format == "json":
            logger.warning(
                "No ingestion sources were found. This could be normal if none have been created yet."
            )
    except Exception as e:
