    ...
    try:
        ...
        if not args.force:
            ...
            if has_entities:
                ...
                if not args.force:
                    logger.error(
                        "Cannot delete tag with associated entities unless --force is specified"
                    )
                    sys.exit(1)
                else:
                    logger.warning(
                        "Will force delete tag and remove tag associations from entities"
                    )

        # Confirm deletion with user unless --confirm is provided
        if not args.confirm:
            user_input = input(
                f"Are you sure you want to delete tag '{tag_name}'? [y/N]: "
            )
