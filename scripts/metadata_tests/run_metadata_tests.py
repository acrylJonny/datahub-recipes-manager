...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run metadata tests against DataHub entities"
    )
...
class DomainRequiredFieldsTest(MetadataTest):
    ...
    def run(self) -> List[TestResult]:
        ...
        for domain in domains:
            ...
            success, message, details = check_required_fields(
                domain_info, required_fields, domain_urn
            )

            results.append(
                self.create_result(
                    success, message, entity_urn=domain_urn, details=details
                )
            )
            ...
            if (
                "properties" in domain_info
                and "description" in domain_info["properties"]
            ):
...
class GlossaryTermRequiredFieldsTest(MetadataTest):
    ...
    def run(self) -> List[TestResult]:
        ...
        for term in terms:
            ...
            success, message, details = check_required_fields(
                term_info, required_fields, term_urn
            )

            results.append(
                self.create_result(
                    success, message, entity_urn=term_urn, details=details
                )
            )
...
def main():
    ...
    try:
        ...
        test_suite = load_tests(
            client, args.entity_type, args.entity_urn, args.test_config
        )
        ...
        for test_name, test_results in results.items():
            for result in test_results:
                if not result.success and result.severity in [
                    TestSeverity.ERROR,
