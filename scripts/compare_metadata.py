...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description="Compare metadata between two DataHub environments")
...
def compare_lists(source_list: List[Dict[str, Any]], target_list: List[Dict[str, Any]], id_field: str = "urn") -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], List[Dict[str, Any]]]:
    """
...
def are_entities_equivalent(source: Dict[str, Any], target: Dict[str, Any]) -> bool:
    ...
    if "properties" in source and "properties" in target and "colorHex" in source["properties"]:
    ...
    for section, data in comparison_results.items():
        if section in ["domains", "glossary", "tags", "structured_properties", "tests"]:
            ...
            if section == "glossary":
                ...
                nodes_missing_in_target = data.get("nodes", {}).get("missing_in_target", [])
                nodes_missing_in_source = data.get("nodes", {}).get("missing_in_source", [])
                nodes_different = data.get("nodes", {}).get("different", [])
                
                terms_missing_in_target = data.get("terms", {}).get("missing_in_target", [])
                terms_missing_in_source = data.get("terms", {}).get("missing_in_source", [])
...
def main():
    ...
    if args.source_token_file:
        ...
        source_token = get_token_from_env("DATAHUB_SOURCE_TOKEN") or get_token_from_env()
    ...
    if args.target_token_file:
        ...
        target_token = get_token_from_env("DATAHUB_TARGET_TOKEN") or get_token_from_env()
    ...
    try:
        if args.compare_type in ["all", "domains"]:
            ...
            missing_in_target, missing_in_source, different = compare_lists(source_domains, target_domains)
            ...
            logger.info(f"Domains comparison complete. Found {len(missing_in_target)} missing in target, {len(missing_in_source)} missing in source, {len(different)} different.")
        ...
        if args.compare_type in ["all", "glossary"]:
            ...
            nodes_missing_in_target, nodes_missing_in_source, nodes_different = compare_lists(source_nodes, target_nodes)
            ...
            terms_missing_in_target, terms_missing_in_source, terms_different = compare_lists(source_terms, target_terms)
            ...
            logger.info(f"Glossary comparison complete. Nodes: {len(nodes_missing_in_target)} missing in target, {len(nodes_missing_in_source)} missing in source, {len(nodes_different)} different. Terms: {len(terms_missing_in_target)} missing in target, {len(terms_missing_in_source)} missing in source, {len(terms_different)} different.")
        ...
        if args.compare_type in ["all", "tags"]:
            ...
            missing_in_target, missing_in_source, different = compare_lists(source_tags, target_tags)
            ...
            logger.info(f"Tags comparison complete. Found {len(missing_in_target)} missing in target, {len(missing_in_source)} missing in source, {len(different)} different.")
        ...
        if args.compare_type in ["all", "properties"]:
            ...
            missing_in_target, missing_in_source, different = compare_lists(source_props, target_props)
            ...
            logger.info(f"Structured properties comparison complete. Found {len(missing_in_target)} missing in target, {len(missing_in_source)} missing in source, {len(different)} different.")
        ...
        if args.compare_type in ["all", "tests"]:
            ...
            missing_in_target, missing_in_source, different = compare_lists(source_tests, target_tests)
            ...
            logger.info(f"Metadata tests comparison complete. Found {len(missing_in_target)} missing in target, {len(missing_in_source)} missing in source, {len(different)} different.")
