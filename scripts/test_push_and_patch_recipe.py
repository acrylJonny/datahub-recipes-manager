...
class TestRecipeIntegration(unittest.TestCase):
    ...
    def test_01_push_recipe(self):
        ...
        self.assertIsNotNone(source, f"Failed to retrieve source {self.source_id} after creation")
        self.assertEqual(source.get("name"), self.source_name, "Source name doesn't match")
    ...
    def test_02_patch_recipe(self):
        ...
        self.assertIsNotNone(updated_source, f"Failed to retrieve updated source {self.source_id}")
        ...
        try:
            ...
            expected_profiling = not current_recipe.get("pipeline_config", {}).get("profiling", {}).get("enabled", True)
            actual_profiling = updated_recipe.get("pipeline_config", {}).get("profiling", {}).get("enabled", False)
            
            self.assertEqual(actual_profiling, expected_profiling, "Profiling setting wasn't updated as expected")
        except Exception as e:
    ...
    def test_03_fallback_mechanism(self):
        ...
        with patch.object(self.client, 'execute_graphql', side_effect=Exception("Forced GraphQL error")):
            ...
            self.assertIsNotNone(final_source, f"Failed to retrieve source after REST fallback")
            self.assertEqual(final_source.get("name"), f"{self.source_name} Updated", "Name wasn't updated via REST fallback")
            self.assertTrue(final_source.get("config", {}).get("debugMode"), "Debug mode wasn't updated via REST fallback")
    ...
    def test_04_cleanup(self):
        ...
        # Verify deletion
        source = self.client.get_ingestion_source(self.source_id)
        self.assertIsNone(source, f"Source {self.source_id} still exists after deletion")
