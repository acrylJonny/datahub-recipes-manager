...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Update GitHub workflow files with intelligent descriptions in comments."
    )
    parser.add_argument(
        "--dir",
...
def format_description_comment(workflow_info):
    ...
    if workflow_info["environments"]:
        comment_lines.append(
            f"# Environments: {', '.join(workflow_info['environments'])}"
        )

    required_inputs = [i for i in workflow_info["inputs"] if i.get("required", False)]
    if required_inputs:
        input_descriptions = [
            f"{i['name']} ({i['description']})" for i in required_inputs
        ]
    ...
    comment_lines.append(
        f"# Complexity: {workflow_info['jobs']} jobs, {workflow_info['steps']} steps"
    )
...
def update_workflow_file(file_path, dry_run=False):
    ...
    try:
        ...
        auto_desc_pattern = re.compile(
            r"# ={20,}\n# .*\n# ={20,}\n# AUTO-GENERATED DESCRIPTION:.*?# ={20,}",
            re.DOTALL,
        )
        ...
        if dry_run:
            ...
            if new_content == content:
                ...
                preview = (
                    new_content[:300] + "..." if len(new_content) > 300 else new_content
                )
                print(preview)
        else:
            if new_content != content:
