...
def patch_ingestion_source(
    ...
    try:
        ...
        if recipe_config is None and schedule is None:
            logger.error("No updates specified. Please provide either a recipe or schedule to update.")
            return False
...
def parse_args():
    ...
    parser.add_argument(
        "--source-id",
        required=True,
        help="ID of the ingestion source to patch"
    )
    
    parser.add_argument(
        "--recipe-file",
        help="Path to the recipe file (YAML or JSON)"
    )
