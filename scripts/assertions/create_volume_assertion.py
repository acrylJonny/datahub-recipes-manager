...
def parse_args():
    ...
    parser.add_argument(
        ...
        "-op",
        required=True,
        choices=["EQUAL_TO", "GREATER_THAN", "GREATER_THAN_OR_EQUAL_TO", "LESS_THAN", "LESS_THAN_OR_EQUAL_TO", "BETWEEN"],
...
def main():
    ...
    if args.operator == "BETWEEN":
        if not args.min_value or not args.max_value:
            logger.error("Both --min-value and --max-value are required for BETWEEN operator")
            sys.exit(1)
    else:
        if not args.value:
