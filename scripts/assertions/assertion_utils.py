...
def load_assertion_from_file(file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if "assertion" in data:
            ...
            logger.warning("Assertion definition not found in expected format, using entire file content")
            return data
...
def save_assertion_to_file(assertion: Dict[str, Any], file_path: str, pretty_print: bool = True) -> bool:
    """
...
def get_assertion_field_path(assertion: Dict[str, Any]) -> Optional[str]:
    ...
    # Field assertion
    if ("fieldAssertion" in info and "fieldValuesAssertion" in info["fieldAssertion"] and 
            "field" in info["fieldAssertion"]["fieldValuesAssertion"]):
