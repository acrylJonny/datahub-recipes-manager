...
def parse_args():
    ...
    list_parser.add_argument(
        "--limit", type=int, default=100, help="Maximum number of policies to list"
    )
    list_parser.add_argument(
        "--start", type=int, default=0, help="Starting offset for pagination"
    )
    ...
    create_parser.add_argument(
        "--description", default="", help="Description of the policy"
    )
    create_parser.add_argument(
        "--type",
        default="METADATA",
        choices=["METADATA", "PLATFORM"],
        help="Type of policy",
    )
    create_parser.add_argument(
        "--state",
    ...
    create_parser.add_argument(
        "--resources",
        help='JSON list of resources (e.g., \'[{"type":"dataset","resource":"*"}]\')',
    )
    create_parser.add_argument(
        "--privileges", help="JSON list of privileges (e.g., '[\"VIEW_ENTITY_PAGE\"]')"
    )
    create_parser.add_argument(
        "--actors", help="JSON object of actors (e.g., '{\"allUsers\":true}')"
    )
    ...
    update_parser.add_argument(
        "--state", choices=["ACTIVE", "INACTIVE"], help="New state for the policy"
    )
    update_parser.add_argument(
        "--resources",
        help='JSON list of resources (e.g., \'[{"type":"dataset","resource":"*"}]\')',
...
def show_export_info():
    ...
    print(
        "python scripts/export_policy.py --server http://datahub:8080 --token your-token --output-dir policies/"
    )
...
def main():
    ...
    datahub_server = os.environ.get("DATAHUB_SERVER") or os.environ.get(
        "DATAHUB_GMS_URL"
    )
    if not datahub_server:
        logger.error(
            "DATAHUB_SERVER or DATAHUB_GMS_URL environment variable is not set"
        )
        sys.exit(1)
    ...
    if args.action == "list":
        ...
        if not policies:
            ...
            for policy in policies:
                print(
                    f"ID: {policy.get('id')}, Name: {policy.get('name')}, "
