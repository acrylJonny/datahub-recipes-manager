...
def import_policy(
    ...
    for policy in existing_policies:
        if policy.get("name") == policy_name:
            ...
            logger.info(f"Found existing policy with name '{policy_name}' (ID: {policy.get('id')})")
            break
    ...
    if existing_policy:
        ...
        logger.warning(f"Policy already exists: {policy_name}. Use --skip-existing or --force-update.")
        return False
    ...
    if result:
        logger.info(f"Successfully created policy: {policy_name} with ID: {result.get('id')}")
        return True
    else:
...
def main():
    ...
    parser.add_argument("--input-dir", help="Directory containing policy files to import")
    ...
    parser.add_argument("--skip-existing", action="store_true", help="Skip policies that already exist")
    parser.add_argument("--force-update", action="store_true", help="Update policies that already exist")
    ...
    logger.info(f"Successfully imported {success_count} out of {len(policy_files)} policies")
