...
def load_domain_from_file(file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if "domain" in data:
            ...
            logger.warning("Domain definition not found in expected format, using entire file content")
            return data
...
def create_update_definition(args, existing_domain: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if args.remove_parent:
        ...
        domain["parentDomains"] = {
            "domains": [
                {"urn": args.parent_domain}
            ]
        }
    ...
    if args.add_owners or args.remove_owners:
        ...
        if args.remove_owners:
            ...
            existing_owners = [owner for owner in existing_owners 
                              if "owner" in owner and "urn" in owner["owner"] 
        ...
        if args.add_owners:
            ...
            for urn in add_urns:
                ...
                if any(("owner" in owner and "urn" in owner["owner"] and owner["owner"]["urn"] == urn) 
...
def main():
    ...
    try:
        ...
        logger.info(f"Retrieved existing domain: {existing_domain.get('properties', {}).get('name', args.domain_urn)}")
        ...
        if success:
            logger.info(f"Successfully updated domain: {updated_domain.get('properties', {}).get('name', args.domain_urn)}")
        else:
