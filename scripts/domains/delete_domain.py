...
def main():
    ...
    try:
        ...
        if not args.force:
            ...
            has_children, has_entities = client.check_domain_dependencies(
                args.domain_urn
            )
            ...
            if has_children:
                ...
                if not args.force:
                    logger.error(
                        "Cannot delete domain with child domains unless --force is specified"
                    )
                    sys.exit(1)
                else:
                    logger.warning(
                        "Will force delete domain and potentially orphan child domains"
                    )
            ...
            if has_entities:
                ...
                if not args.force:
                    logger.error(
                        "Cannot delete domain with associated entities unless --force is specified"
                    )
                    sys.exit(1)
                else:
                    logger.warning(
                        "Will force delete domain and remove domain associations from entities"
                    )

        # Confirm deletion with user unless --confirm is provided
        if not args.confirm:
            user_input = input(
                f"Are you sure you want to delete domain '{domain_name}'? [y/N]: "
            )
