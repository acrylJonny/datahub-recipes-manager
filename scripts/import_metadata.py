...
def validate_metadata(metadata: Dict[str, Any], import_type: str) -> bool:
    ...
    if import_type == "all":
        ...
        required_sections = ["domains", "glossary", "tags", "structured_properties", "tests"]
        missing_sections = [s for s in required_sections if s not in metadata]
        
        if missing_sections:
            logger.warning(f"Metadata package is missing sections: {', '.join(missing_sections)}")
        ...
        if not isinstance(glossary, dict) or "nodes" not in glossary or "terms" not in glossary:
            logger.error("Invalid glossary structure: should contain 'nodes' and 'terms'")
            return False
    ...
    if args.import_type == "all":
        ...
        try:
            if args.import_type == "domains":
                ...
                logger.info(f"Importing {len(nodes)} glossary nodes and {len(terms)} glossary terms")
