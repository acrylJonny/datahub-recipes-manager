...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Sync metadata between DataHub environments"
    )
...
class MetadataSyncer:
    ...
    def sync_glossary_term(self, term_urn: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.use_deterministic_urns:
                ...
                deterministic_urn = self.get_deterministic_urn(
                    term_data, "glossaryTerm"
                )

                if deterministic_urn and deterministic_urn != original_urn:
                    logger.info(
                        f"Remapping glossary term URN: {original_urn} -> {deterministic_urn}"
                    )
    ...
    def sync_glossary_node(self, node_urn: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.use_deterministic_urns:
                ...
                deterministic_urn = self.get_deterministic_urn(
                    node_data, "glossaryNode"
                )

                if deterministic_urn and deterministic_urn != original_urn:
                    logger.info(
                        f"Remapping glossary node URN: {original_urn} -> {deterministic_urn}"
                    )
    ...
    def sync_tag(self, tag_urn: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.use_deterministic_urns:
                ...
                if deterministic_urn and deterministic_urn != original_urn:
                    logger.info(
                        f"Remapping tag URN: {original_urn} -> {deterministic_urn}"
                    )
    ...
    def sync_domain(self, domain_urn: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.use_deterministic_urns:
                ...
                if deterministic_urn and deterministic_urn != original_urn:
                    logger.info(
                        f"Remapping domain URN: {original_urn} -> {deterministic_urn}"
                    )
    ...
    def sync_structured_properties(self, entity_urn: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                ...
                or "properties"
                not in source_result["data"]["entity"]["structuredProperties"]
            ):
                return {
                    "status": "error",
            ...
            properties = source_result["data"]["entity"]["structuredProperties"][
                "properties"
            ]
            ...
            for prop_name, prop_value in properties.items():
                if not self.dry_run:
                    ...
                    target_result = self.target_client.execute_graphql(
                        mutation, mutation_variables
                    )
            ...
            logger.error(
                f"Error syncing structured properties for entity {entity_urn}: {str(e)}"
            )
            return {
                "status": "error",
    ...
    def sync_all_tags(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not tags:
                return [
                    {"status": "info", "message": "No tags found in source environment"}
                ]
            ...
            return [{"status": "error", "message": f"Error syncing all tags: {str(e)}"}]
    ...
    def sync_all_domains(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            return [
                {"status": "error", "message": f"Error syncing all domains: {str(e)}"}
            ]
...
def main():
    ...
    try:
                ...
                if args.include_properties:
                    for result in term_results:
                        if result.get("status") == "success" and "urn" in result:
                            prop_result = syncer.sync_structured_properties(
                                result["urn"]
                            )
                            results["results"].append(prop_result)
        ...
        success_count = sum(
            1 for r in results["results"] if r.get("status") == "success"
        )
        ...
        # Print summary
        logger.info(
            f"Sync completed: {success_count} succeeded, {error_count} failed, {info_count} informational"
        )

        if error_count > 0:
            logger.warning(
                "Some entities failed to sync. Check the results for details."
            )
            if args.dry_run:
                logger.info(
                    "This was a dry run. No changes were made to the target environment."
                )
            sys.exit(1)
        else:
