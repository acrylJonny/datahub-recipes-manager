    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if any_glossary_api_working:
                ...
                self.stdout.write("1. The DataHub instance may not have the glossary module enabled")
                self.stdout.write("2. Your authentication token may not have permission to access glossary data")
                self.stdout.write("3. The API endpoints may have changed in your DataHub version")
            ...
            if nodes:
                self.stdout.write(self.style.SUCCESS(f"✅ Successfully retrieved {len(nodes)} glossary nodes"))
                if verbose and nodes:
                    self.stdout.write("First node sample:")
                    self.stdout.write(json.dumps(nodes[0], indent=2))
            else:
                self.stdout.write(self.style.WARNING("⚠️ No glossary nodes were returned (empty list)"))
                self.stdout.write("This could mean either:")
                self.stdout.write("1. There are no glossary nodes in your DataHub instance")
            ...
            if terms:
                if isinstance(terms, dict):
                    ...
                    self.stdout.write(self.style.SUCCESS(f"✅ Successfully retrieved terms for {len(terms)} parent nodes ({num_terms} total terms)"))
                else:
                    self.stdout.write(self.style.SUCCESS(f"✅ Successfully retrieved {len(terms)} glossary terms"))
                ...
                if verbose and terms:
                    if isinstance(terms, dict):
                        ...
                        self.stdout.write(f"Terms: {json.dumps(terms[first_parent][0], indent=2)}")
                    else:
                        self.stdout.write("First term sample:")
                        self.stdout.write(json.dumps(terms[0], indent=2))
            else:
                self.stdout.write(self.style.WARNING("⚠️ No glossary terms were returned"))
                self.stdout.write("This could mean either:")
                self.stdout.write("1. There are no glossary terms in your DataHub instance")
                self.stdout.write("2. The API calls are failing silently")
                
        except Exception as e:
            self.stdout.write(self.style.ERROR(f"Error testing glossary connection: {str(e)}")) 