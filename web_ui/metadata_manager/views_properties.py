        ...
        try:
            ...
            if connected and client:
                ...
                try:
                    ...
                    logger.debug(f"Fetched {len(remote_properties) if remote_properties else 0} remote properties")
                    ...
                    local_property_urns = set(properties.values_list('deterministic_urn', flat=True))
                    
                    # Process properties by comparing local and remote
                    for prop in properties:
                        prop_urn = str(prop.deterministic_urn)
                        remote_match = next((p for p in remote_properties if p.get('urn') == prop_urn), None)
                    ...
                    remote_only_properties = [p for p in remote_properties if p.get('urn') not in local_property_urns]
                    
                    logger.debug(f"Categorized properties: {len(synced_properties)} synced, {len(local_properties)} local-only, {len(remote_only_properties)} remote-only")
            ...
            return render(request, 'metadata_manager/properties/list.html', {
                'error': str(e),
                'page_title': 'DataHub Structured Properties'
            })
    ...
    def post(self, request):
        ...
        try:
            ...
            for i in range(allowed_value_count):
                ...
                description = request.POST.get(f'allowed_value_description_{i}', '').strip()
            ...
            deterministic_urn = get_full_urn_from_name("structuredProperty", qualified_name)
            
            # Check if property with this URN already exists
            if StructuredProperty.objects.filter(deterministic_urn=deterministic_urn).exists():
                ...
                if property.sync_status != 'LOCAL_ONLY':
                    try:
                        ...
                        if remote_property:
                            ...
                            remote_definition = remote_property.get('definition', {}) or {}
                            remote_name = remote_definition.get('displayName', '')
                            remote_description = remote_definition.get('description', '')
                            
                            if property.name != remote_name or property.description != remote_description:
                                needs_sync = True
                            
                            # Compare settings
                            remote_settings = remote_property.get('settings', {}) or {}
                            if (property.show_in_search_filters != remote_settings.get('showInSearchFilters', True) or
                                property.show_as_asset_badge != remote_settings.get('showAsAssetBadge', True) or
                                property.show_in_asset_summary != remote_settings.get('showInAssetSummary', True) or
                                property.show_in_columns_table != remote_settings.get('showInColumnsTable', False) or
                                property.is_hidden != remote_settings.get('isHidden', False)):
                                needs_sync = True
                        ...
                        logger.warning(f"Error fetching remote property information: {str(e)}")
    ...
    def post(self, request, property_id):
        ...
        try:
            ...
            changed = (
                ...
                property.description != description or
                property.show_in_search_filters != show_in_search_filters or
                property.show_as_asset_badge != show_as_asset_badge or
                property.show_in_asset_summary != show_in_asset_summary or
                property.show_in_columns_table != show_in_columns_table or
        ...
        try:
            ...
            if property.sync_status == 'LOCAL_ONLY':
                ...
                if urn:
                    ...
                    messages.success(request, f"Property '{property.name}' created in DataHub successfully")
                else:
                    messages.error(request, f"Failed to create property '{property.name}' in DataHub")
                ...
                if success:
                    ...
                    messages.success(request, f"Property '{property.name}' updated in DataHub successfully")
                else:
                    messages.error(request, f"Failed to update property '{property.name}' in DataHub")
            else:
                messages.info(request, f"Property '{property.name}' is already synced with DataHub")
    ...
    def post(self, request, only_post=False):
        ...
        try:
            ...
            for remote_property in remote_properties:
                ...
                existing_property = StructuredProperty.objects.filter(deterministic_urn=urn).first()
                ...
                try:
                    ...
                    entity_types = [et.get('type') for et in entity_types_info if et.get('type')]
                    ...
                    if existing_property:
                        ...
                        existing_property.show_in_search_filters = show_in_search_filters
            ...
            if imported_count > 0 or updated_count > 0:
                messages.success(request, f"Successfully imported {imported_count} new properties and updated {updated_count} existing properties")
            else:
