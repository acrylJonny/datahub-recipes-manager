...
class DataHubConnectionMiddleware:
    ...
    def _load_datahub_connection(self, request):
        ...
        if server_url:
            try:
                client = DataHubRestClient(server_url=server_url, token=token if token else None)
                is_connected = client.test_connection()
                request.datahub_connection = {
                    'connected': is_connected,
                    'message': "Successfully connected to DataHub" if is_connected else "Failed to connect to DataHub",
                    'error': None if is_connected else "Could not establish connection to DataHub server. Please check your settings."
                }
