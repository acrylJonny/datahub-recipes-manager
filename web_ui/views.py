...
def script_list(request):
    ...
    for filename in os.listdir(script_dir):
        if filename.endswith(".py") and not filename.startswith("__"):
            ...
            scripts.append(
                {"name": script_name, "description": description, "filename": filename}
            )
...
def run_script(request, script_name):
    ...
    try:
        ...
        cmd = [
            "python",
            "-c",
            f"import sys; sys.path.append('.'); from scripts.{script_name} import parse_args; parser = parse_args(); print(json.dumps({{action.dest: {{ 'help': action.help, 'required': action.required, 'default': action.default }} for action in parser._actions if action.dest != 'help'}}))",
        ]
    ...
    return render(
        request,
        "run_script.html",
        {"script_name": script_name, "parameters": parameters},
    )
...
def script_result(request, result_id):
    ...
    return render(
        request, "script_result.html", {"result": result, "artifacts": artifacts}
    )
...
def export_result(request, result_id):
    ...
    data = {
        ...
        "completed_at": result.script_run.completed_at.isoformat()
        if result.script_run.completed_at
        else None,
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="{result.script_run.script_name}_{result_id}.json"'
    )
    return response
...
def view_artifact(request, artifact_id):
    ...
    if artifact.content_type.startswith("image/"):
        ...
        return render(
            request, "view_artifact.html", {"artifact": artifact, "content": content}
        )
    else:
        # For other file types, suggest download
        return render(
            request, "view_artifact.html", {"artifact": artifact, "download_only": True}
        )
...
def list_scripts(request):
    ...
    for script_file in scripts_dir.glob("*.py"):
        ...
        scripts.append({"name": name, "path": script_file, "description": description})
...
def list_policies(request):
    ...
    try:
        ...
        return render(
            request,
            "policies/list.html",
            {"policies": policies, "output": result.stdout, "error": result.stderr},
        )
    except Exception as e:
        return render(request, "policies/list.html", {"policies": [], "error": str(e)})
...
def edit_policy(request, policy_id):
    ...
    try:
        ...
        return render(
            request,
            "policies/edit.html",
            {"policy": policy, "output": result.stdout, "error": result.stderr},
        )
    except Exception as e:
        return render(
            request,
            "policies/edit.html",
            {"policy": {"id": policy_id}, "error": str(e)},
        )
...
def list_tests(request):
    ...
    for test_file in test_dir.glob("*.sh"):
        tests.append({"name": test_file.name, "path": test_file})
...
def run_test(request, test_name):
    ...
    try:
        # Run the test script
        result = subprocess.run(
            ["bash", str(test_path)], capture_output=True, text=True, cwd=BASE_DIR
        )
        return render(
            request,
            "tests/result.html",
        ...
        return render(
            request,
            "tests/result.html",
            {"test_name": test_name, "error": str(e), "exit_code": 1},
        )
...
def api_run_status(request, run_id):
    ...
    try:
        ...
        try:
            ...
            result = {"output": result_obj.output, "error": result_obj.error}
        ...
        data = {
            ...
            "started_at": script_run.started_at.isoformat()
            if script_run.started_at
            else None,
            "completed_at": script_run.completed_at.isoformat()
            if script_run.completed_at
            else None,
...
def github_secrets(request):
    ...
    if request.GET.get("environment"):
        return redirect(
            f"{reverse('github_secrets')}?environment={request.GET.get('environment')}"
        )
