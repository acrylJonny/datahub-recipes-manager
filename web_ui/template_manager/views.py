...
def recipe_template_export(request, template_id):
    ...
    response = HttpResponse(json.dumps(template_content, indent=2), content_type='application/json')
    response['Content-Disposition'] = f'attachment; filename="{template.name.replace(" ", "_").lower()}.json"'
    return response
