...
class MutationStore:
    ...
    def transform_entity(self, entity: Model, environment_config: Optional[Dict[str, Any]] = None) -> Model:
        """
    ...
    def _get_entity_type(self, entity: Model) -> Optional[str]:
        ...
        from web_ui.metadata_manager.models import (
            GlossaryNode, GlossaryTerm, Tag, Domain, Assertion
        )
    ...
    def _transform_glossary_node(self, node: Model, environment_config: Dict[str, Any]) -> Model:
        """
        ...
        if environment_config.get('add_environment_to_description', False) and node.description:
            env_name = environment_config.get('name', 'Unknown')
            node.description = f"[{env_name}] {node.description}"
        
        return node
    
    def _transform_glossary_term(self, term: Model, environment_config: Dict[str, Any]) -> Model:
        """
        ...
        if environment_config.get('name') and hasattr(term, 'term_source') and term.term_source:
    ...
    def _transform_domain(self, domain: Model, environment_config: Dict[str, Any]) -> Model:
        """
    ...
    def _transform_assertion(self, assertion: Model, environment_config: Dict[str, Any]) -> Model:
        """
                            ...
                            if isinstance(entity, str) and 'urn:li:' in entity:
                                ...
                                if len(parts) > 3:
                                    ...
                                    if not entity_id.startswith(f"{env_name}_"):
                                        config['entities'][i] = f"urn:li:{entity_type}:{env_name}_{entity_id}"
    ...
    def _transform_container(self, container: Model, environment_config: Dict[str, Any]) -> Model:
        """
    ...
    def _transform_dataset(self, dataset: Model, environment_config: Dict[str, Any]) -> Model:
        """
