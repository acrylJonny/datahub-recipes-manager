...
def _run_script_sync(script_run):
    ...
    try:
        ...
        result = ScriptResult.objects.create(
            script_run=script_run, output=stdout, error=stderr
        )
        ...
        # Create result record
        ScriptResult.objects.create(
            script_run=script_run, output="", error=f"Error running script: {str(e)}"
        )
        ...
        if file_path.is_file():
            ...
            if metadata_path.exists():
                try:
                    ...
                    logger.warning(
                        f"Error reading metadata for {file_path.name}: {str(e)}"
                    )
