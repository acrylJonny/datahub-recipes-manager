...
class WorkflowAnalyzer:
    ...
    def get_workflows(self, branch="main"):
        ...
        try:
            ...
            if "github.com" in self.base_url:
                ...
                "dev.azure.com" in self.base_url or "visualstudio.com" in self.base_url
            ):
                # Azure DevOps API
            ...
            if (
                "github.com" in self.base_url
                or "dev.azure.com" not in self.base_url
                and "gitlab.com" not in self.base_url
                and "bitbucket.org" not in self.base_url
            ):
                ...
                for file_info in files:
                    ...
                    if not (
                        file_info["name"].endswith(".yml")
                        or file_info["name"].endswith(".yaml")
                    ):
                        continue
                    ...
                    if file_response.status_code != 200:
                        logger.error(
                            f"Failed to fetch workflow file {file_info['name']}: {file_response.status_code} {file_response.text}"
                        )
                        continue
                    ...
                    if not isinstance(file_data, dict) or "content" not in file_data:
                        logger.error(
                            f"Invalid file content response for {file_info['name']}"
                        )
                        continue
    ...
    def analyze_workflow(workflow_content: str) -> Dict:
        ...
        try:
            ...
            jobs_count, steps_count, actions = WorkflowAnalyzer._count_jobs_and_steps(
                workflow_data
            )
            ...
            complexity_score = WorkflowAnalyzer._calculate_complexity(
                workflow_data, actions, steps_count
            )

            # Estimate execution time
            estimated_time = WorkflowAnalyzer._estimate_execution_time(
                workflow_data, actions, steps_count
            )
    ...
    def _calculate_complexity(
        workflow_data: Dict, actions: List[str], steps_count: int
    ) -> int:
    ...
    def _estimate_execution_time(
        ...
        for job_id in graph:
            sequential_chain_length = max(
                sequential_chain_length, find_longest_path(job_id)
            )
    ...
    def _generate_description(
        ...
        if actions:
            ...
            if (
                any(da in " ".join(actions).lower() for da in deployment_actions)
                or "deploy" in name.lower()
            ):
            ...
            if (
                any(ba in " ".join(actions).lower() for ba in build_actions)
            ...
            if (
                any(ta in " ".join(actions).lower() for ta in test_actions)
            ...
            if "manage-ingestion" in name.lower() or any(
                "ingestion" in a.lower() for a in actions
            ):
                description_parts.append("manages DataHub ingestion sources")
            elif "manage-policy" in name.lower() or any(
                "policy" in a.lower() for a in actions
            ):
                description_parts.append("manages DataHub policies")
            elif "manage-env" in name.lower() or any(
                ("env" in a.lower() or "var" in a.lower()) for a in actions
            ):
        ...
        if dependencies:
            description_parts.append(
                f"uses a {complexity_term} structure with dependent jobs"
            )
        else:
            if jobs_count > 1:
                description_parts.append(
                    f"uses a {complexity_term} structure with {jobs_count} parallel jobs"
                )
            else:
        ...
        description = ". This workflow ".join(
            part.capitalize() for part in description_parts
        )
