...
class RepositoryLoader:
    ...
    def load_all(cls):
        ...
        logger.info(
            f"Repository data load complete: environments={environments_loaded}, "
            ...
            for env_dir in templates_dir.iterdir():
                if env_dir.is_dir():
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Skipping templates for unknown environment: {env_name}"
                        )
    ...
    def _load_env_vars_template(cls, template_file, environment=None):
        ...
        try:
            ...
            logger.info(
                f"{'Created' if created else 'Updated'} template: {template.name} ({template_file})"
            )
            return True
    ...
    def _load_recipe_template(cls, template_file):
        ...
        try:
            ...
            logger.info(
                f"{'Created' if created else 'Updated'} recipe template: {template.name} ({template_file})"
            )
            return True

        except Exception as e:
            logger.error(
                f"Error loading recipe template from {template_file}: {str(e)}"
            )
            return False
            ...
            for env_dir in params_dir.iterdir():
                if env_dir.is_dir():
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Skipping instances for unknown environment: {env_name}"
                        )
            ...
            for env_dir in instances_dir.iterdir():
                if env_dir.is_dir():
                    ...
                    try:
                        env = Environment.objects.get(name=env_name)
                        for instance_file in env_dir.glob("*.yml"):
                            if cls._load_env_vars_instance_from_recipe(
                                instance_file, env
                            ):
                                instances_loaded += 1
                    except Environment.DoesNotExist:
    ...
    def _load_env_vars_instance(cls, instance_file, environment):
        ...
        try:
            ...
            if isinstance(instance_data, dict):
                ...
                if recipe_type:
                    template = EnvVarsTemplate.objects.filter(
                ...
                if "parameters" in instance_data:
                    for key, value in instance_data["parameters"].items():
                        variables_dict[key] = {"value": value, "isSecret": False}
            ...
            instance, created = EnvVarsInstance.objects.update_or_create(
                ...
                defaults={
                    ...
                    or f"Environment variables for {name} in {environment.name}",
                },
            )

            logger.info(
                f"{'Created' if created else 'Updated'} env vars instance: {instance.name} ({instance_file})"
            )
            return True

        except Exception as e:
            logger.error(
                f"Error loading env vars instance from {instance_file}: {str(e)}"
            )
            return False
    ...
    def _load_env_vars_instance_from_recipe(cls, instance_file, environment):
        ...
        try:
            ...
            if not instance_data.get("parameters") and not instance_data.get(
                "secret_references"
            ):
                return False
            ...
            if variables_dict:
                ...
                instance, created = EnvVarsInstance.objects.update_or_create(
                    ...
                    defaults={
                        ...
                        "variables": json.dumps(variables_dict),
                        "description": description
                        or f"Environment variables extracted from {name} in {environment.name}",
                    },
                )

                logger.info(
                    f"{'Created' if created else 'Updated'} env vars instance from recipe: {instance.name} ({instance_file})"
                )
                return True
            ...
            logger.error(
                f"Error loading env vars from recipe instance {instance_file}: {str(e)}"
            )
            return False
            ...
            for env_dir in instances_dir.iterdir():
                if env_dir.is_dir():
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Skipping recipe instances for unknown environment: {env_name}"
                        )
    ...
    def _load_recipe_instance(cls, instance_file, environment):
        ...
        try:
            ...
            if isinstance(instance_data, dict):
                ...
                if recipe_type:
                    template = RecipeTemplate.objects.filter(
                        recipe_type=recipe_type
                    ).first()

                    # If there are multiple templates, try to find the best match
                    if not template:
                        templates = RecipeTemplate.objects.filter(
                            recipe_type=recipe_type
                        )
                ...
                env_vars_instance = EnvVarsInstance.objects.filter(
                    name=instance_name, environment=environment
                ).first()

                # If we can't find one with the matching name, just use a compatible one
                if not env_vars_instance and recipe_type:
                    env_vars_instance = EnvVarsInstance.objects.filter(
                        recipe_type=recipe_type, environment=environment
                    ).first()

            if not template:
                logger.warning(
                    f"Can't find matching template for recipe instance: {name} (type: {recipe_type})"
                )
                return False
            ...
            instance, created = RecipeInstance.objects.update_or_create(
                ...
                defaults={
                    ...
                    "description": description
                    or f"Recipe instance for {name} in {environment.name}",
                },
            )

            logger.info(
                f"{'Created' if created else 'Updated'} recipe instance: {instance.name} ({instance_file})"
            )
            return True

        except Exception as e:
            logger.error(
                f"Error loading recipe instance from {instance_file}: {str(e)}"
            )
            return False
