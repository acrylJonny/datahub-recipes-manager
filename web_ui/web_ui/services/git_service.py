...
class GitService:
    ...
    def setup_api_settings(self):
        ...
        if self.provider == "github":
            ...
            self.api_base = (
                f"{base_url}/repos/{self.settings.username}/{self.settings.repository}"
            )
            self.headers = {
            ...
            logger.info(
                f"Initialized GitHub API service for repository: {self.settings.username}/{self.settings.repository}"
            )
            ...
            if len(org_project) != 2:
                logger.error(
                    f"Invalid Azure DevOps username format: {self.settings.username}"
                )
                self.api_base = None
                self.headers = {}
                return
            ...
            logger.info(
                f"Initialized Azure DevOps API service for repository: {self.settings.repository}"
            )
            ...
            logger.info(
                f"Initialized GitLab API service for repository: {self.settings.username}/{self.settings.repository}"
            )
            ...
            if "bitbucket.org" in (self.settings.base_url or "bitbucket.org"):
                ...
                auth_token = base64.b64encode(
                    f"{self.settings.username}:{self.settings.token}".encode()
                ).decode()
                self.headers = {
                    "Authorization": f"Basic {auth_token}",
                    "Content-Type": "application/json",
                ...
                logger.info(
                    f"Initialized Bitbucket Cloud API service for repository: {self.settings.username}/{self.settings.repository}"
                )
            else:
                # Bitbucket Server
                ...
                logger.info(
                    f"Initialized Bitbucket Server API service for repository: {self.settings.username}/{self.settings.repository}"
                )
        else:
    ...
    def make_api_request(
        self, method, endpoint, params=None, json_data=None, headers=None
    ):
    ...
    def get_file_content(self, path: str, ref: str = None) -> Optional[Dict]:
        ...
        if self.provider == "github":
            ...
            if response and response.status_code == 200:
                ...
                logger.error(
                    f"HTTP error: {response.status_code} {response.text} for URL: {response.url}"
                )
            ...
            response = self.make_api_request(
                "GET", f"/items?path={path}", params=params
            )
            if response and response.status_code == 200:
            ...
            params = {"ref": ref}
            response = self.make_api_request(
                "GET", f"/repository/files/{path_encoded}", params=params
            )
            if response and response.status_code == 200:
                data = response.json()
                return {
    ...
    def delete_file(
        self, path: str, branch: str = None, message: str = "Delete file"
    ) -> bool:
        """
        ...
        if self.provider == "github":
            # GitHub API
            data = {"message": message, "sha": file_info.get("sha"), "branch": branch}
            response = self.make_api_request(
                "DELETE", f"/contents/{path}", json_data=data
            )
            if response and response.status_code == 200:
                return True
            ...
            response = self.make_api_request(
                "GET", f"/refs?filter=heads/{branch}&api-version=6.0"
            )
            if not response or response.status_code != 200:
                return False
            ...
            push_data = {
                "refUpdates": [
                    {"name": f"refs/heads/{branch}", "oldObjectId": object_id}
                ],
                "commits": [
                    {
                        "comment": message,
                        "changes": [{"changeType": "delete", "item": {"path": path}}],
                    }
                ],
            }

            response = self.make_api_request(
                "POST", "/pushes?api-version=6.0", json_data=push_data
            )
            if response and (
            ...
            response = self.make_api_request(
                "DELETE", f"/repository/files/{path_encoded}", json_data=data
            )
            ...
            if "bitbucket.org" in (self.settings.base_url or "bitbucket.org"):
                ...
                # Bitbucket Server
                # Get latest commit on branch
                response = self.make_api_request("GET", "/branches")
                if not response or response.status_code != 200:
                    return False
                ...
                response = self.make_api_request("POST", "/commits", json_data=data)
                if response and response.status_code in [200, 201, 204]:
                    return True
    ...
    def commit_file(
        ...
        if self.provider == "github":
            ...
            response = self.make_api_request(
                method, f"/repository/files/{path_encoded}", json_data=data
            )
            if response and response.status_code in [200, 201]:
                return True
    ...
    def create_pull_request(
        ...
        if not base_branch:
            ...
            if self.provider == "github":
                ...
                if response and response.status_code == 200:
                    base_branch = (
                        response.json()
                        .get("defaultBranch", "refs/heads/main")
                        .replace("refs/heads/", "")
                    )
                else:
                    base_branch = "main"
        ...
        if self.provider == "github":
            ...
            response = self.make_api_request(
                "POST", "/pullrequests?api-version=6.0", json_data=data
            )
            if response and response.status_code == 201:
                pr_data = response.json()
                # Create PR record
            ...
            if "bitbucket.org" in (self.settings.base_url or "bitbucket.org"):
                ...
                response = self.make_api_request(
                    "POST", "/pullrequests", json_data=data
                )
                if response and response.status_code == 201:
                    pr_data = response.json()
                    # Create PR record
                ...
                response = self.make_api_request(
                    "POST", "/pull-requests", json_data=data
                )
                if response and response.status_code == 201:
                    pr_data = response.json()
                    # Create PR record
    ...
    def list_files(
        self, path: str = "", branch: str = None, recursive: bool = False
    ) -> List[Dict]:
        """
        ...
        if self.provider == "github":
                    ...
                    for item in items:
                        ...
                        if recursive and item.get("type") == "dir":
                            ...
                            sub_files = self.list_files(
                                item.get("path"), branch, recursive
                            )
            ...
            response = self.make_api_request(
                "GET", f"/items?path={path}&scopePath={path}", params=params
            )
            if response and response.status_code == 200:
                data = response.json()
                ...
                if response and response.status_code == 200:
                    ...
                    for d in dirs:
                        result.append(
                            {
                                ...
                                "path": d.get("path"),
                                "type": "dir"
                                if d.get("type") == "commit_directory"
                                else "file",
                        ...
                        if recursive and d.get("type") == "commit_directory":
                            sub_files = self.list_files(
                                d.get("path"), branch, recursive
                            )
                            result.extend(sub_files)
            else:
                # Bitbucket Server
                params = {"at": branch, "type": "ALL" if recursive else "BOTH"}
                path_str = f"?path={path}" if path else ""
                response = self.make_api_request(
                    "GET", f"/files{path_str}", params=params
                )
                if response and response.status_code == 200:
                    data = response.json()
                    ...
                    if diff.strip():  # Only add if diff is not empty
                        ...
                        try:
                            ...
                            if file_content and "content" in file_content:
                                ...
                                if (
                                    file_content.get("encoding") == "base64"
                            ...
                            logger.debug(
                                f"Error getting file size for {file['path']}: {e}"
                            )
                        ...
                        if f["type"] == "file":
                            ...
                            if diff.strip():  # Only add if diff is not empty
                                ...
                                try:
                                    ...
                                    logger.debug(
                                        f"Error getting file size for {f['path']}: {e}"
                                    )
        ...
        try:
            ...
            try:
                ...
                if e.response.status_code == 404:
                    ...
                    logger.debug(
                        f"File {path} not found in {base_branch} branch (new file)"
                    )
                else:
                    # Only log other HTTP errors as errors
                    logger.error(
                        f"HTTP error getting file from {base_branch}: {str(e)}"
                    )
            except Exception as e:
            ...
            if current_content and "content" in current_content:
                if current_content.get("encoding") == "base64":
                    current_text = base64.b64decode(current_content["content"]).decode(
                        "utf-8"
                    )
                else:
            ...
            if base_content and "content" in base_content:
                if base_content.get("encoding") == "base64":
                    base_text = base64.b64decode(base_content["content"]).decode(
                        "utf-8"
                    )
                else:
    ...
    def commit_yaml_file(
        self, data: Dict, path: str, branch: str = None, message: str = None
    ) -> bool:
        """
