...
class GitHubIntegration:
    ...
    @classmethod
    def create_pull_request(cls, recipe_id, recipe_name, recipe_content, description=""):
        """
        ...
        try:
            ...
            ref_response = requests.get(
                cls.get_api_url(f"git/refs/heads/{default_branch}"),
                headers=headers
            )
        ...
        try:
            file_check = requests.get(
                cls.get_api_url(f"contents/{file_path}"),
                headers=headers
            )
        ...
        try:
            ...
            pr = GitHubPR(
                ...
                title=pr_title,
                description=description or f"Update recipe {recipe_name} (ID: {recipe_id})"
            )
            pr.save()
    ...
    def get_pr_status(cls, pr_number):
        ...
        try:
            ...
            pr_response = requests.get(
                cls.get_api_url(f"pulls/{pr_number}"),
                headers=headers
            )
            pr_response.raise_for_status()
