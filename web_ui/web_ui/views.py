...
def recipe_delete(request, recipe_id):
    ...
    try:
        ...
        if result:
            ...
            if is_ajax:
                return JsonResponse({'success': False, 'error': 'Failed to delete recipe'})
...
def env_vars_instance_push_github(request, instance_id):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Environment variable instance with ID {instance_id} not found")
            return JsonResponse({
                'success': False,
        ...
        logger.error(f"Error pushing environment variables to GitHub: {str(e)}", exc_info=True)
        return JsonResponse({
            'success': False,
            'error': str(e)
        })
...
def env_vars_template_push_github(request, template_id):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Environment variable template with ID {template_id} not found")
            return JsonResponse({
                'success': False,
...
def environments(request):
    ...
    return render(request, 'environments/list.html', {
        'title': 'Environments',
        'environments': envs
    })
...
def environment_delete(request, env_id):
    ...
    if request.method == 'POST':
        if recipe_count == 0 and env_vars_count == 0:
            ...
            messages.error(request, f"Cannot delete environment '{environment.name}' - it is in use by {recipe_count} recipe instances and {env_vars_count} environment variable instances")
            
        return redirect('environments')
        
    return render(request, 'environments/delete.html', {
...
def github_push_changes(request):
    ...
    try:
        # Commit the changes
        commit_message = request.POST.get('commit_message', 'Update recipes and policies')
...
def github_create_pr(request):
    ...
    if not has_changes:
        messages.warning(request, 'No staged changes found. Stage changes before creating a PR.')
        return redirect('github_repo')
    ...
    try:
        ...
        if pr_result and isinstance(pr_result, dict) and 'number' in pr_result:
            ...
            messages.success(request, f'Pull request #{pr_number} created successfully.')
...
def github_branches(request):
    ...
    return render(request, 'github_branches.html', {
        'settings': settings,
        'branches': branches,
...
def github_create_branch(request):
    ...
    base_branch = request.POST.get('base_branch', settings.current_branch or 'main').strip()
...
def github_sync_recipe(request, recipe_id):
    ...
    if request.method == 'POST':
        # Get form data
        commit_message = request.POST.get('commit_message', f'Update recipe: {recipe.name}')
        branch_name = request.POST.get('branch_name')
        ...
        try:
            ...
            if result:
                messages.success(request, f'Recipe "{recipe.name}" staged for commit to branch {settings.current_branch}.')
                return redirect('github_repo')
            else:
        ...
        return render(request, 'github_sync.html', {
            'recipe': recipe,
            'settings': settings,
            'branches': branches,
            'current_branch': settings.current_branch,
...
def github_sync_all_recipes(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if success_count > 0:
                messages.success(request, f'Successfully staged {success_count} recipes for commit.')
                if error_count > 0:
        ...
        return render(request, 'github_sync_all.html', {
            'settings': settings,
            'branches': branches,
            'current_branch': settings.current_branch,
            'recipe_count': RecipeTemplate.objects.count(),
...
def github_sync_policy(request, policy_id):
    ...
    if request.method == 'POST':
        # Get form data
        commit_message = request.POST.get('commit_message', f'Update policy: {policy.name}')
        branch_name = request.POST.get('branch_name')
        ...
        try:
            ...
            if result:
                messages.success(request, f'Policy "{policy.name}" staged for commit to branch {settings.current_branch}.')
                return redirect('github_repo')
            else:
        ...
        return render(request, 'github_sync_policy.html', {
            'policy': policy,
            'settings': settings,
            'branches': branches,
            'current_branch': settings.current_branch,
...
def github_sync_all_policies(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if success_count > 0:
                messages.success(request, f'Successfully staged {success_count} policies for commit.')
                if error_count > 0:
                    messages.warning(request, f'Failed to stage {error_count} policies.')
            else:
...
def github_revert_staged_file(request):
    ...
    try:
        ...
        if result:
            messages.success(request, f'Successfully reverted staged changes for "{file_path}".')
        else:
            messages.error(request, f'Failed to revert staged changes for "{file_path}".')
    except Exception as e:
...
def policy_push_github(request, policy_id):
    ...
    try:
        ...
        if result and result.get('success'):
            # Success response
            logger.info(f"Successfully staged policy {policy.id} to Git branch {current_branch}")
            return JsonResponse({
                'success': True,
