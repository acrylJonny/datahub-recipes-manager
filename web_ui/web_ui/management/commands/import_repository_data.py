...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(f"- {results['template_vars']} environment variable templates")
            self.stdout.write(f"- {results['recipe_templates']} recipe templates")
            self.stdout.write(f"- {results['env_vars_instances']} environment variable instances")
            ...
            self.stdout.write(self.style.SUCCESS(f"Import complete. Loaded {total_loaded} items in total."))
