    ...
    try:
        ...
        if provider_type == "azure_devops" and "/" not in username:
            return JsonResponse(
                {
                    "success": False,
                    "error": "Azure DevOps username must be in format: organization/project",
                }
            )
        ...
        if provider_type == "github":
            ...
            if response.status_code == 200:
                ...
                error_message = (
                    f"HTTP {response.status_code}: Unable to access repository"
                )
                try:
            ...
            if "bitbucket.org" in (base_url or "bitbucket.org"):
                ...
                if not base_url:
                    return JsonResponse(
                        {
                            "success": False,
                            "error": "Base URL is required for Bitbucket Server",
                        }
                    )

                api_base = base_url.rstrip("/")
                repo_url = (
                    f"{api_base}/rest/api/1.0/projects/{username}/repos/{repository}"
                )
            ...
            if not base_url:
                return JsonResponse(
                    {
                        "success": False,
                        "error": "Base URL is required for custom Git providers",
                    }
                )
    ...
    try:
        ...
        if branch_description:
            pr_result = GitIntegration.create_pr_from_staged_changes(
                title=f"New branch: {branch_name}", description=branch_description
            )
            ...
            if pr_result and pr_result.get("success"):
                ...
                messages.warning(
                    request, "Branch created but pull request creation failed"
                )
...
def github_update_pr_status(request, pr_number):
    ...
    if not GitSettings.is_configured():
        return JsonResponse(
            {"success": False, "error": "Git integration not configured"}
        )
