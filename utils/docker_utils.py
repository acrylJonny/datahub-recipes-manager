...
def is_testing_environment():
    ...
    testing_indicators = [
        'TESTING', 'TEST_MODE', 'DATAHUB_TEST_ENV', 'CI', 'GITHUB_ACTIONS'
    ]
...
def should_apply_docker_networking():
    ...
    return is_testing_environment() and (
        is_in_docker() or 
        os.environ.get('DOCKER_COMPOSE_MODE', '').lower() in ['true', '1', 'yes']
    )
...
def resolve_docker_host(host: str, default_port: Optional[int] = None) -> Dict[str, Any]:
    ...
    # we need to use localhost instead
        ...
        logger.info(f"Not in Docker but using Docker service name '{host}'. Using 'localhost' instead.")
                    ...
                    for line in hosts_file:
                        if 'host-gateway' in line:
                            ...
                            if parts and parts[0]:
                                ...
                                logger.info(f"Using host gateway {parts[0]} to access host from Docker container")
                                break
