...
class DataHubMetadataApiClient:
    ...
    def _create_pipeline_context(self) -> PipelineContext:
        ...
        try:
            ...
            logger.debug(f"DataHubGraph verify_ssl set to: {self.verify_ssl}")
            ...
            logger.error(f"Error initializing DataHubGraph client: {str(e)}")
            raise
            ...
            if (
                ...
                if parent_domains:
                    ...
                    domains_aspect = DomainsClass(
                        domains=[parent for parent in parent_domains]
                    )
    ...
    def create_glossary_node(self, node_data: Dict[str, Any]) -> Optional[str]:
        """Create a glossary node using GraphQL mutation.

        Args:
            node_data (Dict[str, Any]): The glossary node data containing name, description, and parent_urn

        Returns:
            Optional[str]: The URN of the created glossary node if successful, None otherwise
        """
        try:
            mutation = """
            ...
            result = self.context.graph.execute_graphql(mutation, variables)

            if result and "data" in result and "createGlossaryNode" in result["data"]:
                return result["data"]["createGlossaryNode"]["urn"]

            logger.error(f"Failed to create glossary node: {result}")
            return None
