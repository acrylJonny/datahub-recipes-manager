...
try:
    from utils.docker_utils import update_connection_params, should_apply_docker_networking
...
def apply_docker_networking(recipe: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if 'source' in updated_recipe and 'config' in updated_recipe['source']:
        ...
        if source_type in ['postgres', 'postgresql', 'mysql', 'mssql', 'oracle', 'db2', 'snowflake']:
            if isinstance(source_config, dict) and 'host' in source_config:
                ...
                logger.info(f"Updated {source_type} connection parameters for Docker: {source_config.get('host')}:{source_config.get('port')}")
