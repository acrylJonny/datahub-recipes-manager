...
class DataHubMetadataClient:
    ...
    def get_root_glossary_nodes(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            nodes_data = (
                result.get("data", {}).get("getRootGlossaryNodes", {}).get("nodes", [])
            )
    ...
    def get_root_glossary_terms(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            terms_data = (
                result.get("data", {}).get("getRootGlossaryTerms", {}).get("terms", [])
            )
    ...
    def list_all_tags(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            tags = [
                ...
                if result.get("entity", {}).get("type") == "TAG"
            ]
    ...
    def list_structured_properties(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            props = [
                result.get("entity")
                for result in search_results
                if result.get("entity", {}).get("type") == "STRUCTURED_PROPERTY"
            ]
    ...
    def list_metadata_tests(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            tests = [
                result.get("entity")
                for result in search_results
                if result.get("entity", {}).get("type") == "TEST"
            ]
    ...
    def export_domain(
        ...
        try:
            ...
            if include_entities:
                ...
                entities_variables = {
                    "urn": domain_urn,
                    "input": {"start": 0, "count": 1000, "query": "*"},
                }

                entities_result = self.client.execute_graphql(
                    entities_query, entities_variables
                )

                if (
                    entities_result
                    and "data" in entities_result
                    and "domain" in entities_result["data"]
                ):
                    entities_data = (
                        entities_result["data"]["domain"]
                        .get("entities", {})
                        .get("searchResults", [])
                    )
                    domain_data["entities"] = [
                        r.get("entity", {}) for r in entities_data
                    ]
    ...
    def export_all_metadata(self, output_file: str) -> bool:
        ...
        try:
            ...
            metadata_package = {
                "version": "1.0",
                "domains": domains,
                "glossary": {"nodes": glossary_nodes, "terms": glossary_terms},
                "tags": tags,
